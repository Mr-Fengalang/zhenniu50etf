...
class Markettable extends Component {
          ...
          ws.onmessage = (evt) => {
              ...
              .then(
                  (res) => {
                      ...
                      if(res.pushType === 4) {
                        if(res.data.etfName==="50ETF"){
                          this.props.gettype4(res.data)
                        }
                      }
                      if (res.pushType === 5) {
                      }
  ...
  render() {
    ...
    }, 
    {
      ...
      render:(text, row, index)=> {
        return (
          <span onClick={() => this.props.greet(row.buyHy)} className={((row.buyHy.lastPrice-row.buyHy.preClosePrice)/row.buyHy.preClosePrice)>0?"zhang":"die"}>
              {(((row.buyHy.lastPrice-row.buyHy.preClosePrice)/row.buyHy.preClosePrice).toFixed(4) * 100).toFixed(2)}%
          </span> 
        )
      }
    },
 ...
 
    {
      ...
      render:(text, row, index)=> {
        return(
          <span onClick={() => this.props.greet(row.sellHy)} className={((row.sellHy.lastPrice-row.sellHy.preClosePrice)/row.sellHy.preClosePrice)>0?"zhang":"die"}>
              {row.sellHy.lastPrice}
          </span> 
          )  
      }
    },     {
      ...
      render:(text, row, index)=> {
        return (
          <span onClick={() => this.props.greet(row.sellHy)}  className={((row.sellHy.lastPrice-row.sellHy.preClosePrice)/row.sellHy.preClosePrice)>0?"zhang":"die"}>
              {(((row.sellHy.lastPrice-row.sellHy.preClosePrice)/row.sellHy.preClosePrice).toFixed(4) * 100).toFixed(2)}%
          </span> 
        )
      }
    },  {
    ...
    return (
      <div className="Markettable markettablebox" style={{height:document.body.clientHeight-243}}>
            <Table 
            className="rengou"
                pagination={false}
                columns={columns} 
                    ...
                    return {
                      onClick: () => {
                        ...
                        this.setState({
                          ...
                          // 点击行
                      onDoubleClick:()=>{
                        if (document.querySelectorAll('.anticon-fullscreen')[0]) {
                          setTimeout(
                            document.querySelectorAll('.anticon-fullscreen')[0].click(),1000
                          )
                          }
                      }
                    };
...
//   }
// )
export default connect(mapStateToProps,null)(Markettable)